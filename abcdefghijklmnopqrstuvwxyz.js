...
/**
 * ======================================================
 * MESIN IKLAN V14 (Tidak perlu diubah)
 * ======================================================
 */
(function() {
    ...
    styleSheet.innerText = `
        ...
        /* Latar belakang dinamis dengan efek blur tetap dipertahankan */
        .iklan-bg-blur {
        ...
        /* BARU: TEMA VERTIKAL (KE BAWAH) */
        .iklan-interstitial-konten {
            overflow-y: auto; /* Scroll ke bawah */
            flex-grow: 1;
            padding: 80px 16px 16px 16px; /* Jarak dari atas dan sisi */
            box-sizing: border-box;
        }
        .iklan-interstitial-konten::-webkit-scrollbar { display: none; } /* Sembunyikan scrollbar */
        ...
        .iklan-vertical-item:last-child {
            margin-bottom: 0;
        }
        .iklan-vertical-item img {
            width: 100%;
            display: block;
            border-radius: 12px;
        ...
        /* Navigasi atas tetap dengan efek blur */
        .iklan-interstitial-topbar { 
        ...
        /* --- STYLE UNTUK TOP-BAR (Iklan jenis pertama) --- */
    ...
    document.addEventListener('DOMContentLoaded', () => {
        PENGATURAN_IKLAN.forEach(iklan => {
            ...
            try {
                if (iklan.jenisPenempatan === 'top-bar') {
                    pasangIklanTopBar(iklan);
                } else if (iklan.jenisPenempatan === 'interstitial-gambar') {
                    setTimeout(() => pasangIklanInterstitial(iklan), iklan.tunda * 1000);
                }
            } catch(e) { console.error(e); }
        });
    });

    function pasangIklanInterstitial(iklan) {
        ...
        const bannersHtml = iklan.banners.map(banner => `
            <a href="${banner.link}" target="_blank" class="iklan-vertical-item">
                <img src="${banner.gambarUrl}" alt="Iklan Banner">
            </a>
        `).join('');

        overlay.innerHTML = `
            <div class="iklan-bg-blur" style="background-image: url('${bgImageUrl}');"></div>
            <div class="iklan-interstitial-topbar">
                <button id="iklan-interstitial-close">&times;</button>
                <a href="${iklan.tombolUtama.link}" target="_blank" class="iklan-interstitial-learnmore">${iklan.tombolUtama.teks}</a>
            </div>
            <div class="iklan-interstitial-konten">
                ${bannersHtml}
            </div>
        `;
        ...
        const closeBtn = document.getElementById('iklan-interstitial-close');
    ...
    function pasangIklanTopBar(iklan) {
